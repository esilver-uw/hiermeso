...
get_delta <- function(size, groups, treatment_g) {
  ...
  delta <- 0.9 * (sqrt(group.size * N.SIZE) * size) / SIGMA
  return(delta)
}

# Simulation goal: for each group, take p-value, direct e-value, and calibrate p-value to e-value. 
# We compare the size of the direct e-value to the calibrated e-value
first_sim <- function(groups, theta, treatment_g, size, iterations) {
  theta_prime <- perturb_parameter_matrix(theta = theta, groups = groups, g = treatment_g, size = size)
  
  dir_vals <- rep(0, iterations)
  cal_vals <- rep(0, iterations)
  p_vals <- rep(0, iterations)
  
  for (k in 1:iterations) {
    A1 <- sample_network(theta = theta, N)
    A2 <- sample_network(theta = theta_prime, N)
    
    p_val <- p_value(A1, A2, groups, treatment_g)
    
    dir_vals[k] <- e_value_dir(A1, A2, groups, treatment_g, get_delta(size, groups, treatment_g))
    cal_vals[k] <- e_value_cal(p_val)
    p_vals[k] <- p_val
  }
  
  return(c(mean(dir_vals), mean(cal_vals), mean(p_vals)))
}

first_sim(GROUPS, THETA, "res_2_group_2", 1, 20)

# Second simulation goal: for each group, take direct e-values from multiple deltas near the oracle. 
second_sim <- function(groups, theta, treatment_g, size) {
  theta_prime <- perturb_parameter_matrix(theta = theta, groups = groups, g = treatment_g, size = size)
  
  A1 <- sample_network(theta = theta, N)
  A2 <- sample_network(theta = theta_prime, N)
  
  deltas <- (1 + (0:20 - 10)/100) * get_delta(size, groups, treatment_g)
  e_vals <- list()
  
  for (d in deltas) {
    e_vals[[as.character(d)]] <- e_value_dir(A1, A2, groups, treatment_g, d)
  }
...
# Overestimating size
second_sim(GROUPS, THETA, "res_2_group_2", 5)
