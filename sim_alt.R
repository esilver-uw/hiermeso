...
simulation <- function(groups, alpha, theta, perturb_g, sizes, selector = 2) {
  ...
  for (size in sizes) {
    ...
    # Baseline
    for (g in groups[[4]]$res_Group[groups[[4]]$Resolution == 1]) {
      e_vals_baseline <- append(e_vals, e_value(A1, A2, groups, g))
    }

    detections[[as.character(size)]] <- elp(e_vals, groups, alpha)[,selector]
    
    detections_baseline[[as.character(size)]] <- elp(e_vals_baseline, groups[groups[[4]]$Resolution == 1], alpha)[,selector]
  }
  return(list(detections, detections_baseline))
}
...
detections <- groupwise_sim(GROUPS, 0.05, THETA, SIZES)
